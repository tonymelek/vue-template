<template>
    <div class="container">
      <div class="card p-3 mt-5 max-width-500">
      <h2 class="mb-3">Payment Form</h2>
      <form id="payment-form">
    <label for="amount">Amount</label>
    <input type="number" class="form-control mt-1" id="amount"  v-model="amount" @focus="renderCard">
      <div id="card-container"></div>
      <button id="card-button" class="btn btn-primary w-100" 
      :class="amount>0?'':'d-none'"
    ...
    <div id="payment-status-container" :class="statusConatinerClassList"></div>
    </div>
</template>

<script>
    import { appId,locationId,isLive } from '../constants';
    let card;
    export default {
        name:'payments-page',
        computed:{
            statusConatinerClassList(){
                return this.statusContainerClasses.join(' ');
            },
            paymentButton(){
              if(!this.loading){
                if(this.cardButtonDisabled){
        ...
        methods:{
          removeFromArray(arr,toRemove){
            return arr.filter(el=>el!==toRemove)
          },
          async createPayment(token) {
            const paymentResponse = await fetch('https://px3rqnx5kh.execute-api.ap-southeast-2.amazonaws.com/dev', {
            method: 'POST',
            body:JSON.stringify({
              source_id: token,
              amount_money: {
            ...
            const {statusCode,body}=await paymentResponse.json();
            this.loading=false;
            if (statusCode===200) {
            return body;
            }
      ...
      displayPaymentResults(status) {
        if (status === 'SUCCESS') {
        this.statusContainerClasses=this.removeFromArray(this.statusContainerClasses,'is-failure');
        this.statusContainerClasses.push('is-success');
        } else {
          this.statusContainerClasses=this.removeFromArray(this.statusContainerClasses,'is-success');
          if(!this.statusContainerClasses.includes('is-failure')) this.statusContainerClasses.push('is-failure');
        }
        if(!this.statusContainerClasses.includes('visible')) this.statusContainerClasses.push('visible');
      },
      async  handlePaymentMethodSubmission(event) {
        ...
        async renderCard(){
          if (this.cardRendred) return;
          this.cardRendred=true;
          try {
          const payments = await Square.payments(appId, locationId);
          card = await payments.card();
          await card.attach('#card-container');
        } catch {
          this.statusContainerClasses=[...this.statusContainerClasses,'missing-credentails','visible'] 
        } 
        }
